        ...
        public AttributesMenu(string[] args)
        {
            if (args != null)
            {
                ...
                foreach (string arg in args)
                {
                    ...
                    {
                        StartProcess.StartInfo(AppDomain.CurrentDomain.BaseDirectory + "XtendedMenu.exe", "\"" + ex.Message + Environment.NewLine + ex.StackTrace + Environment.NewLine + ex.Source + Environment.NewLine + ex.GetBaseException() + Environment.NewLine + ex.TargetSite + "\"" + " -catchhandler");
                    }
                    ThePaths = args;
                    FilePath = arg;
